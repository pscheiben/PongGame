...
#include    "informationboard.h"    //life symbols and handling
#include    "gamemenu.h"            //game setting options
               ...
               else
               {
                   while(1) //infinite main loop
                   {
                       ...
                       if(InputUpdatePending)
                       {
                           InputUpdatePending=0;
                           ReadAccX();
                           if(accdx>16) //Accelerometer sensitive enough X Axis
                           {
                               if (xR1 > INF_SLIDE_WIDTH + 1) //avoid overwriting left wall
                               {
                                   xR1=xR1-4;
                           ...
                           if(accdx<-16) //if the elevation angle differs enough X Axis
                           {
                               if (xR1 < LCD_COL-2-HALF_RACKET_SIZE*2-INF_SLIDE_WIDTH) //avoid overwriting right wall
                               {
                                   xR1=xR1+4;
               ...
               else
               {
                   while(1) //infinite main loop
                   {
                       ...
                       if(InputUpdatePending)
                       {
                           InputUpdatePending=0;
                           ReadAccX();
                           if(accdx>16) //if the elevation or angle differs enough X Axis
                           {
                               if (xR1 > INF_SLIDE_WIDTH + 1) //avoid overwriting left wall
                               {
                                   xR1=xR1-2;
               ...
               else
               {
                   while(1) //infinite main loop
                   {
                       ...
                       if(InputUpdatePending)
                       {
                           ...
                           if(accdy>16) //if the elevation or angle differs enough Y Axis
                           {
                               if (xR2 > INF_SLIDE_WIDTH + 1) //avoid overwriting left wall
                               {
                                   xR2=xR2-2; //move racket 2 pixel left
...
void GameStartInit()
{
 ...
 //Initial position of racket 2 player two top side
    ...
    InputUpdatePending = 1; //avoid fast starting ball movement this bug  is noted one of the version
    InputChangePending = 1;
    BallUpdatePending = 1;
    LCDUpdatePending = 1;
...
void LCD_update(void)
{
 ...
 InputChangePending==0; //Does not need to redraw the racket
}

// Drawing a new ball with 5 pixel
...
// For drawing the shade of the ball
...
// Calculate the X-axis difference
void ReadAccX(void)
{
...
void halBoardInit(void)
{
    ...
    // Additional pins needs to be tied
